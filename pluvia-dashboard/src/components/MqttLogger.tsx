...
export function MqttLogger({ onExpand, isFull }: MqttLoggerProps) {
	...
	useEffect(() => {
		...
		client.on("connect", () => {
			...
			client.subscribe("pluvia/#", (err) => {
				if (!err) {
					...
					pushSysLog('🎧 Auto-inscrito no coringa "pluvia/#" para debug global.');
				}
			});
		});
		...
		client.on("message", (topic, message) => {
			...
			try {
				const parsed = JSON.parse(displayPayload);
				displayPayload = JSON.stringify(parsed, null, 2);
			} catch (e) {
				console.log("ERRO DO MQTTLOGGER AQUI Ó: ", e);
			}
			...
			<div className="logger-content-split">
					...
					<div className="logger-controls">
						...
						<div className="topics-section">
							<span className="section-subtitle">
								Tópicos Inscritos ({topicosInscritos.length}):
							</span>
				...
				<div className="terminal-wrapper">
					...
					<div className="terminal-window" ref={terminalWindowRef}>
						{logs.length === 0 ? (
							<div className="terminal-empty">Aguardando mensagens...</div>
						) : (
							logs.map((log, i) => (
								<div
									key={i}
								...
								>
									...
									>
										...
										<strong
											className="log-topic"
											style={{ color: log.topic.includes("DASHBOARD") ? "#fbbf24" : "#60a5fa" }}
										>
											{log.topic}
										</strong>
									</div>
